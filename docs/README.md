...
Active가 없는 상태에서 "안녕"을 보내면 한국어 대화 답변이 나오고 영어/코드블록/NAME: 접두가 없어야 합니다.
Active가 없는 상태에서 "요약해줘"를 보내면 파일 첨부 필요 안내와 1)~3) 다음 행동이 보여야 합니다.
Active가 있는 상태에서 분석을 실행하면 기존 결과 카드 뒤에 한국어 해설 메시지(핵심 발견+다음 행동 1~2개)가 추가되어야 합니다.
같은 오류/안내 문구가 연속으로 누적되면 중복 방지 로직 미동작으로 간주합니다.
